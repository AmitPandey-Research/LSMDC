...
class FaceAttention3(nn.Module):
    ...
    def forward(self, fc_feat, img_feat, label_feat, memory, face, face_mask):
        ...
        for i in range(memory.size(1)):
            ...
            '''
            ipdb> face.shape, fc_feat.shape, label_feat.shape
            (torch.Size([64, 10, 512]), torch.Size([64, 10, 256]), torch.Size([64, 10, 512]))
            '''
        ...
        inputs = inputs.view(-1, self.dim1 + self.dim2).unsqueeze(1) # torch.Size([640, 1536])
        o = self.linear2(torch.tanh(self.linear1(inputs)))
