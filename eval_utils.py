    ...
    while True:
        ...
        with torch.no_grad():
            ...
            if split == 'test' or eval_accuracy:
                predicted_characters, predicted_genders = gen_model(fc_feats, sent_num_batch, face_feats, face_masks, face_segment_ids, captions, masks, position_ids, segment_ids, blank_indexes, gt_captions,
                                                                    gt_masks, blank_masks, bert_emb, slots, slot_masks, slot_size, mode='predict')
                torch.cuda.synchronize()
                #ipdb.set_trace()
                #predicted_characters, predicted_genders = parse_predictions(predicted_characters, predicted_genders, slots)
                predicted_characters = predicted_characters.data.cpu().numpy().astype('int')
